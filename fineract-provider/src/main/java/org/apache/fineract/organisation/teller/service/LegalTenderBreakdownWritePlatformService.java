...
public class LegalTenderBreakdownWritePlatformService {

    private final CashLegalTenderLineRepository cashLegalTenderLineRepository;

    @Transactional
    public void saveLines(final List<CashLegalTenderLine> lines) {
        if (lines == null || lines.isEmpty()) {
            return;
        }
        this.cashLegalTenderLineRepository.saveAll(lines);
    }
}
