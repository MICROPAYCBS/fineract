...
public final class ContactTypeCommandFromApiJsonDeserializer {
    ...
    public static final String RESOURCE = "ContactType";

    private static final Set<String> SUPPORTED_PARAMETERS = new HashSet<>(Arrays.asList(TYPE_CODE, TYPE_NAME, EXAMPLE, VALIDATION_REGEX,
            MANDATORY, DISPLAY_ORDER, STATUS));
    ...
    public void validateForUpdate(final Long contactTypeId, final String json) {
        validateSupported(json);
        final List<ApiParameterError> errors = new ArrayList<>();
        final DataValidatorBuilder validator = new DataValidatorBuilder(errors).resource(RESOURCE);
        validator.reset().parameter("id").value(contactTypeId).notNull().integerGreaterThanZero();
        final JsonElement element = this.fromApiJsonHelper.parse(json);
    ...
    private void validateCommonFields(final JsonElement element, final DataValidatorBuilder validator, final boolean create) {
        if (create || this.fromApiJsonHelper.parameterExists(TYPE_CODE, element)) {
            final String typeCode = this.fromApiJsonHelper.extractStringNamed(TYPE_CODE, element);
            validator.reset().parameter(TYPE_CODE).value(typeCode).notBlank().notExceedingLengthOf(20);
        }
        if (create || this.fromApiJsonHelper.parameterExists(TYPE_NAME, element)) {
            final String typeName = this.fromApiJsonHelper.extractStringNamed(TYPE_NAME, element);
            validator.reset().parameter(TYPE_NAME).value(typeName).notBlank().notExceedingLengthOf(100);
        }
        ...
        if (this.fromApiJsonHelper.parameterExists(MANDATORY, element)) {
            final Boolean mandatory = this.fromApiJsonHelper.extractBooleanNamed(MANDATORY, element);
            validator.reset().parameter(MANDATORY).value(mandatory).ignoreIfNull().notNull();
        }
