 ...
 * under the License.
 */
package org.apache.fineract.portfolio.industry.serialization;
...
public final class IndustryCommandFromApiJsonDeserializer {
    ...
    public static final String PRIORITY_INDUSTRY = "priorityIndustry";
    public static final String PROHIBITED_INDUSTRY = "prohibitedIndustry";
    ...
    public static final String RESOURCE = "Industry";

    private static final Set<String> SUPPORTED_PARAMETERS = new HashSet<>(Arrays.asList(INDUSTRY_CODE, INDUSTRY_NAME, DESCRIPTION,
            SECTOR_ID, REGULATORY_CODE, RISK_LEVEL, AML_RISK_LEVEL, CREDIT_RISK_LEVEL, PRIORITY_INDUSTRY, PROHIBITED_INDUSTRY,
            REQUIRES_EDD, EXPOSURE_LIMIT, EXPECTED_TURNOVER_MIN, EXPECTED_TURNOVER_MAX, STATUS));
    ...
    public void validateForUpdate(final Long industryId, final String json) {
        validateSupported(json);
        final List<ApiParameterError> errors = new ArrayList<>();
        final DataValidatorBuilder validator = new DataValidatorBuilder(errors).resource(RESOURCE);
        validator.reset().parameter("id").value(industryId).notNull().integerGreaterThanZero();
        final JsonElement element = this.fromApiJsonHelper.parse(json);
    ...
    private void validateCommonFields(final JsonElement element, final DataValidatorBuilder validator, final boolean create) {
        if (create || this.fromApiJsonHelper.parameterExists(INDUSTRY_CODE, element)) {
            final String industryCode = this.fromApiJsonHelper.extractStringNamed(INDUSTRY_CODE, element);
            validator.reset().parameter(INDUSTRY_CODE).value(industryCode).notBlank().notExceedingLengthOf(20);
        }
        if (create || this.fromApiJsonHelper.parameterExists(INDUSTRY_NAME, element)) {
            final String industryName = this.fromApiJsonHelper.extractStringNamed(INDUSTRY_NAME, element);
            validator.reset().parameter(INDUSTRY_NAME).value(industryName).notBlank().notExceedingLengthOf(100);
        }
        ...
        if (this.fromApiJsonHelper.parameterExists(SECTOR_ID, element)) {
            final Long sectorId = this.fromApiJsonHelper.extractLongNamed(SECTOR_ID, element);
            validator.reset().parameter(SECTOR_ID).value(sectorId).ignoreIfNull().integerGreaterThanZero();
        }
