--
-- MICROPAY CBS: General Ledger table report -- department filter (MariaDB / MySQL)
--
        ...
        SELECT p2.as_of_date,
            (SELECT MAX(s.snapshot_date)
             ...
             WHERE s.snapshot_granularity = ''DAILY''
               ...
               AND (s.department_id = p2.department_id OR p2.department_id = -1)
...
FROM (
  ...
  WHERE je.account_id = ${GLAccountNO}
    ...
    AND (COALESCE(je.department_id, 0) = ${departmentId} OR ''-1'' = ''${departmentId}'')
...
ORDER BY lines.entry_date DESC, lines.id DESC'
