--
-- MICROPAY CBS: Trial Balance Table — hybrid snapshot + delta (MariaDB / MySQL)
--
...
WITH params AS (
    SELECT CAST(''${startDate}'' AS DATE) AS start_date,
           ...
           CAST(''${departmentId}'' AS SIGNED) AS department_id,
           ''${currencyId}'' AS currency_id
),
snapshot_dates AS (
    SELECT end_date AS as_of_date FROM params
    UNION ALL
    SELECT DATE_SUB(start_date, INTERVAL 1 DAY) FROM params
),
snapshot_watermarks AS (
    ...
    CROSS JOIN params p
    LEFT JOIN (
        SELECT sd2.as_of_date,
            (SELECT MAX(s.snapshot_date)
             FROM m_gl_balance_snapshot s
             WHERE s.snapshot_granularity = ''DAILY''
               AND s.snapshot_date <= sd2.as_of_date
               AND s.snapshot_date >= DATE_SUB(sd2.as_of_date, INTERVAL 90 DAY)
    ...
    ) baselines ON baselines.as_of_date = sd.as_of_date
),
balances AS (
...
period AS (
    ...
    LEFT JOIN balances s
     ...
     AND s.as_of_date = DATE_SUB(p.start_date, INTERVAL 1 DAY)
